...
# 🔐 MANDATORY: Validate .github/ folder security on every task operation
...
if [ -f "$PROJECT_ROOT/.github/task-management/scripts/github_security_monitor.sh" ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "🚨 Task operations blocked until violations are fixed"
        exit 1
    fi
    echo "✅ .github/ folder security validated"
else
    echo "⚠️  Security monitor not found - proceeding with caution"
fi
